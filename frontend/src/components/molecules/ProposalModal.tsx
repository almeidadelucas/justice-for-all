  ...
  const handleGetProposal = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        const proposal = res.data.proposals.some(
          (p: IProposal) => p.lawyerId === loggedUser?.userId
        );
    ...
    <Modal open={open} onClose={onClose}>
      <ModalBox display="flex" flexDirection="column" rowGap="1rem">
        {loading ? (
          ...
          <>
            <Typography>
              Deseja enviar uma proposta para ser a pessoa responsável por esse
              caso?
            </Typography>
            <Box display="flex" flexDirection="row" columnGap="1rem">
              <Button variant="outlined" onClick={handleSendProposal}>
                Sim
              </Button>

...
import {
  ...
  UserTypes,
  Visions,
} from './ShowCasesPage.interfaces';
import { FilterBy } from '../molecules/FilterBy';

export default function ShowCasesPage() {
  const { loggedUser } = useContext(AuthContext);
  const [vision, setVision] = useState<TVisions>(
    (loggedUser?.type === 'LAWYER'
      ? Visions.LAWYER_CASES
      : Visions.CLIENT_CASES) as TVisions
  );
      ...
      >
        {loggedUser?.type === UserTypes.LAWYER && (
          <SwitchLawyerVision
            ...
            handleChange={(value) => {
              const newVision = (
                value ? Visions.OPENED_CASES : Visions.LAWYER_CASES
              ) as keyof typeof Visions;
              setVision(newVision);
            }}
          />
        )}
